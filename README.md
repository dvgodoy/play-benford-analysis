...
To perform a 2D-Scanning Benford Analysis (SBA) over an image using a given window size (wSize).
...
To return a processed image as a Base64 string, given the percentage [0..1] of pixels with lowest scores at SBA to be discarded (perc) and choise of a white background color (white = 1) instead of black.
